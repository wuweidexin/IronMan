		...
		<TR>
			...
			<TD align="left" valign="top">
				<TABLE border="0" class="top" width="700">
					<TR>
						<TD class="FormTitle" height="2" width="100%"><B>考试计划查询</B>
						</TD>
					</TR>
					<TR>
						<TD>	&nbsp;
				...
				<option value=''>自动批阅</option>
				<option value=''>老师批阅</option>
				-->
				</select>
									</TD>
									<TD>	班级编号：
				...
				<INPUT class=box name="classNum" size=20 maxLength=20 value="">
								...
								<TD>	状态：
									...
									<TD>										
				<select name="examPlanStatus" >
				<option value=''>无</option>
				<c:forEach items="${examplanstatus}" var="ps">
				<option value="${ps.psName}">${ps.psName}</option>
				</c:forEach>
						...
						<TD height="40" valign="top">
							<TABLE align="center" border="0" class="ItemList" height="20" width="96%">
								<TR height="24" align="middle">
												...
												<TD class="ItemTitle" width="18%">	考试时间
												</TD>
												...
												<TD class="ItemTitle" width="14%">	状态
												</TD>
											</TR>
											<c:forEach items="${list}" var="li">
											<TR align="middle" class="ItemBody">
												<TD class="ItemBody" height="20"><a href="examPlanAction!examPlanQueryNum?epqn=${li.planId}">${li.epMyselfId}</a>
												</TD>
												...
												<TD class="ItemBody" align="left"><a href="<%=basePath %>examPlanAction!classNumQuery?classNum=${li.tbClasses.CNum}">${li.tbClasses.CName}</a>
												</TD>
												<TD class="ItemBody">${li.planReadOverTime}
												</TD>
												<TD class="ItemBody">${li.tbPlanStatus.psName }
												</TD>
											</TR>
											</c:forEach>
											<TR align="middle" class="ItemBody">
...
{	
	...
	document.form1.action="examPlanAction!examProjectQuery?newHQL=1"; 
...
function clickButton(){ 
		...
		form1.action=" <%=basePath %>examPlanAction!examProjectQuery?pageMethod="+page;
		form1.submit();
}	
function clickButton1(){ 
	
	    document.location.href='<%=basePath %>examPlanAction!examProjectQuery?pageMethod=first&currentPage=${currentPage}';
}	
function clickButton2(){ 
	
	   document.location.href='<%=basePath %>examPlanAction!examProjectQuery?pageMethod=previous&currentPage=${currentPage}';
}	
function clickButton3(){ 
	
	   document.location.href='<%=basePath %>examPlanAction!examProjectQuery?pageMethod=next&currentPage=${currentPage}';
}	
function clickButton4(){ 
	
	   document.location.href='<%=basePath %>examPlanAction!examProjectQuery?pageMethod=last&currentPage=${currentPage}';
}	
