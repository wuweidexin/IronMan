					...
					<TR>
						<TD>
							<TABLE align="center" border="0" width="90%">
								...
								<TD>	<TEXTAREA class=box  cols="70%" name="auditAdvice" rows=4>请输入审核意见</TEXTAREA> 
									</TD>
								</TR>
							</TABLE>
						</TD>
...
{	
	...
	{
		alert("通过成功！"); 
		form1.action='<%=basePath %>problemManagerAction!toPassAudit?pId=${tbProblem.PId}'; 
		form1.submit();
	}
}
function toDelete()
{	
	if(confirm("确定拒绝题目吗？"))
	{
		form1.action='<%=basePath %>problemManagerAction!toRefuseAudit?pId=${tbProblem.PId}'; 
		form1.submit();
	}
}
function toReturn()
{	
	document.location.href='<%=basePath %>problemManagerAction!returnQuesLibAudit'; 
}
</SCRIPT>