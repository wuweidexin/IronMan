		...
		<TR>
			...
			<TD align="left" valign="top">
				<TABLE border="0" class="top" width="700">
					<TR>
						<TD class="FormTitle" height="2" width="100%"><B>题目查询</B>
						</TD>
					</TR>
					<TR>
						<TD>&nbsp;	
				...
				<option value=${dl.pdlName }>${dl.pdlName }</option>
								...
								<TR>
									...
									<TD>
				                      <INPUT type="text" name="addTbUser" class="box"  value=''>
									</TD>
									<TD>	状态：
									</TD>
									<TD>										
			<select name="asName" > 
				<option value="" selected>全部</option>
				<c:forEach items="${auditStatusList}" var="al">
				<option value="${al.asName}">${al.asName}</option>
				</c:forEach>
				</select>
									</TD>
								</TR>
							...
							<TABLE align="center" border="0" class="ItemList" height="20" width="96%">
								<TR height="24" align="center">
											...
											<TR align="center" class="ItemBody">
												<TD class="ItemBody" height="20">	<a href='<%=basePath %>problemManagerAction!quesLibQuery2?proNum=${pl.col1}'>${pl.col1 }</a>
												</TD>
												...
												<TD class="ItemBody">${pl.tbAuditStatus.asName }
												</TD>
											</TR>
											</c:forEach>
											<TR align="center" class="ItemBody">
...
{	
	form1.action='<%=basePath %>problemManagerAction!quesLibQuery?newHQL=1';
	form1.submit();
}
...
function clickButton(){ 
		...
		form1.action="<%=basePath %>problemManagerAction!quesLibQuery?currentPage="+page;
		form1.submit();
}	
function clickButton1(){ 
	
	    document.location.href='<%=basePath %>problemManagerAction!quesLibQuery?pageMethod=first&currentPage=${currentPage}';
}	
function clickButton2(){ 
	
	   document.location.href='<%=basePath %>problemManagerAction!quesLibQuery?pageMethod=previous&currentPage=${currentPage}';
}	
function clickButton3(){ 
	
	   document.location.href='<%=basePath %>problemManagerAction!quesLibQuery?pageMethod=next&currentPage=${currentPage}';
}	
function clickButton4(){ 
	
	   document.location.href='<%=basePath %>problemManagerAction!quesLibQuery?pageMethod=last&currentPage=${currentPage}';
}	
</SCRIPT>