Ext.define("JS.ColorPicker.colorPickerWin", {
			...
			initComponent : function() {
				...
				cp.name = "color-picker";
				cp.colors = ['FFFFFF', 'FFFFF0', 'FFFFE0', 'FFFF00', 'FFFAFA',
				...
				cp.on('select', me.colorSelect);
				Ext.apply(me, {
							items : cp
						});
				this.callParent(arguments);
			},
			colorSelect : function(picker, selColor) {
				var me = this;
				//alert(selColor);
				if (selColor.length != 6){
					return alert("请输入正确的十六进制颜色码！")
				}
				//color2.bgColor = "#" + selColor;
				selColor = selColor.toLowerCase();
				var b = new Array();
				for (x = 0; x < 3; x++) {
					b[0] = selColor.substr(x * 2, 2)
					b[3] = "0123456789abcdef";
					b[1] = b[0].substr(0, 1);
					b[2] = b[0].substr(1, 1);
					b[20 + x] = b[3].indexOf(b[1]) * 16 + b[3].indexOf(b[2]);
				}
				var result = b[20] + "," + b[21] + "," + b[22];
				Ext.Msg.alert('提示','十六进制值是：'+selColor+'-----RGB颜色值是：' + result);
			}
		})