...
var Calendar = new Class(
			{
				...
				initialize : function(F, B) {
					...
					for ( var C in F) {
						...
						E.button.addClass(this.classes.calendar).addEvent(
								"click", function(I) {
									this.toggle(I)
						...
						if (F.getTag() == "select") {
							if (F.format.test("(y|Y)")) {
								...
								if (E.getLast() < A.getFullYear()) {
									d = new Date(E.getLast(), A.getMonth() + 1,
											0);
				...
				caption : function(G) {
					...
					if (this.options.navigation == 2) {
						var F = new Element("span")
								.addClass(this.classes.month).injectInside(B);
						if (A.prev.month) {
						...
						B.adopt(new Element("span")
								.addClass(this.classes.month).appendText(
										this.options.months[G.month]));
				...
				clicked : function(C, A, B) {
					B.val = (this.value(B) == A) ? null : new Date(B.year,
							B.month, A);
					this.write(B);
					if (!B.val) {
						B.val = this.read(B)
				...
				toggle : function(C) {
					...
					if (C.visible) {
						...
						if (!this.calendar.coord) {
							this.calendar.coord = this.calendar
									.getCoordinates()
						}
				...
				values : function(F) {
					var D, A, H;
					F.els.each( function(I) {
						if (I.getTag() == "select") {
							if (I.format.test("(y|Y)")) {
								D = [];
								I.getChildren().each( function(K) {
									var J = this.unformat(K.value, I.format);
									if (!D.contains(J[0])) {
										D.push(J[0])
									}
								}, this);
